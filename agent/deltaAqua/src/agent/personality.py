...
class PersonalityHandler:
    ...
    def __init__(self, llm, chat_memory, memory_client):
        self.llm = llm
    ...
    async def handle_user_identity(self, user_message: str, user_id: str, conversation_id: str, chat_name: str) -> dict:
        ...
        known_from_session = []
        try:
            events_resp = self.memory_client.list_events(
            ...
            for event in events_resp.get('events', []):
                for item in event.get('payload', []):
                    if isinstance(item, dict) and 'conversational' in item:
                        ...
                        # Only scan user messages for personal disclosures
                        if role == 'USER' and text and text != user_message:
                            known_from_session.append(text)
        except Exception as e:
            logger.debug(f"Could not fetch session events for user_identity: {e}")

        if known_from_session:
            response = (
                "Based on what you've shared with me in this conversation:\n\n"
                + "\n".join(f"- {msg}" for msg in known_from_session[-5:])
            )
        else:
