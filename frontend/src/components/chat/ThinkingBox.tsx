              ...
              <div key={category} className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-3 pl-4 relative before:absolute before:inset-y-1 before:left-1 before:w-[2px] before:bg-border/50">
                  {grouped[category].map((log, logIdx) => {
                    const isLastLog =
                      isLastCategory && logIdx === grouped[category].length - 1;
                    const isFinishedLog = isComplete || !isLastLog;

                    return (
                      <div
                        key={logIdx}
                        className="flex items-start gap-3 relative w-full min-w-0"
                      >
                        {isFinishedLog ? (
                        ...
                        <span
                          className={cn(
                            "flex-1 min-w-0 leading-snug break-words whitespace-normal",
                            isFinishedLog
